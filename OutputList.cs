...
namespace CruiseProcessing
{
    public class OutputList : CreateTextFile
    {
        #region
                ...
                switch (whichPage)
                {
                    ...
                    case 2:     //  page three -- payment units
                        foreach (CuttingUnitDO cud in cList)
                        {
                            ...
                            ArrayList strataList = Global.BL.GetUnitStrata(cud.Code);
            ...
            private void WriteLogList(StreamWriter strWriteOut, reportHeaders rh, IEnumerable<LogDO> logList, IEnumerable<TreeDO> tList,
                                        ref int pageNumb)
            {
                ...
                foreach (TreeDO tdo in tList)
                {
                    ...
                    List<LogDO> currentLogs = LogMethods.GetLogRecords(logList, (long)tdo.Tree_CN).ToList();
            ...
            private void WriteFallBuckScale(StreamWriter strWriteOut, reportHeaders rh, List<TreeDO> justFBS, ref int pageNumb, IEnumerable<LogStockDO> lsList)
            {
                foreach (TreeDO tdo in justFBS)
                {
                    //  find logs for this tree
                    IEnumerable<LogStockDO> fbsLogs = lsList.Where(lsdo => lsdo.Tree_CN == tdo.Tree_CN);
            ...
            private void WriteLogStock(StreamWriter strWriteOut, reportHeaders rh, IEnumerable<LogStockDO> lsList, ref int pageNumb)
            {
                ...
                foreach (LogStockDO lsdo in lsList)
                {
                    ...
                    double STacres = Utilities.ReturnCorrectAcres(lsdo.Tree.Stratum.Code, (long)lsdo.Tree.Stratum_CN);
            ...
            private void WriteSampleGroup(StreamWriter strWriteOut, reportHeaders rh, IEnumerable<SampleGroupDO> sgList, ref int pageNumb)
            {
